...
按「最小可用、随 dsh 演进」原则推进，以下为当前已知缺口（修复进行中）：

- **崩溃重启尚未生效**：supervisor 设计为意外退出后重启一次，但重启判定存在
  缺陷，dsh 就绪后崩溃不会触发重启（未就绪阶段的失败处理正常）。
- **导航后无错误 UI**：错误/引导页只在初始加载阶段有效；dsh 就绪后被导航覆盖，
  之后若 dsh 退出或出错，窗口停留在死页面（点 Web UI 关机按钮后的表现同此）。
- **stderr 未持续排空**：dsh 长时间运行产生大量 stderr 输出时有阻塞风险。
- **无文件日志**：诊断信息仅输出到 stdout/stderr，Finder 启动时不可见
  （PLAN 规划的 `~/Library/Logs/<identifier>/` 尚未落地）。
- **改端口需手编 settings.json**；保存失败提示用的 `alert()` 在 WKWebView 下
  不显示。
- **注入按钮依赖 DOM 锚点**（搜索按钮 aria-label / 侧边栏文案），dsh 改版可能
  失效；归档范围含 blank 会话，与侧边栏可见集合略有差异。
- 就绪探测基于双 200，极端情况下仍可能把端口上的其他静态服务误认为 dsh。
