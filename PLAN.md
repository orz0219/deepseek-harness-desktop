...
## 附录 A — 实现增量记录（Slice 1 之后的偏离，随代码更新）

PLAN 是设计文档；以下是实现中落地、但 PLAN 正文未覆盖的行为，避免文档漂移：

- **注入式「归档所有对话」按钮 + 归档管理面板**（`main.rs` INJECT_JS +
  `archive_ops.rs`）：锚定 dsh 搜索按钮 `aria-label` 注入；批量归档走 dsh 自身
  RPC；撤销归档 / 物理删除由启动器原生命令直接编辑 `~/.dsh` 存储
  （原子写 + 备份 + id 校验），经 capabilities 授权给远环 dsh 页面调用，
  操作后按需重启 dsh 生效。dsh 改版可能使 DOM 锚点失效（已知取舍）。
- **快速路径连接**：配置端口上已有健康 dsh（双 200 + `/api` 404 签名）时直接
  连接，不重复拉起；连接后仍持续探测，失联走与崩溃相同的恢复路径。
- **窗口行为**：点关闭 = 隐藏到程序坞（后台 dsh 继续跑）；程序坞 Reopen /
  单实例二次启动 = 重新显示并聚焦。
- **supervisor 实语义**：统一为「建立连接 → 看护直到失联/退出/被取代」，最多
  MAX_ATTEMPTS 次；未就绪类失败（端口冲突等）不自动重启（门禁 E）；干净退出
  （Web UI 关机按钮）不重启，显示说明页 + 重启按钮。
- **错误页恢复**：出错时 webview 导航回启动器内置页（origin 启动时捕获），
  不再停留在死掉的 dsh 页面。
- **stderr 持续排空**：独立读线程维持管道空闲并保留 ~16KiB 尾部用于诊断，
  避免 dsh 长跑写满管道缓冲假死。
- **文件日志**：`~/Library/Logs/com.deepseek.harness.desktop/launcher.log`
  （时间戳行，镜像 stderr）；启动命令、env 快照、候选列表、生命周期迁移均落盘。
- **设置 UI**：支持 dsh 路径 + 端口（≥1024）+ 可执行位校验；保存失败用页面内
  联提示（WKWebView 无原生 alert）。
